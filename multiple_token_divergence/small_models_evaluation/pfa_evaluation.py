...
from small_models_evaluation.custom_generation import generate_from_recipe, ASCII_LOGIT_MASK, \
    generate_next_token_mtp_with_blending
...
def decode_token_by_token(tokens, tokenizer):
    ...
    if type(tokenizer) is ASCIITokenizer:
        ...
        if type(token_list[0]) == list:
            tokens_by_tokens = [[token_to_str[t] for t in seq] for seq in token_list]
        else:
            tokens_by_tokens = [token_to_str[t] for t in token_list]
        return tokens_by_tokens
...
def generate_per_token_losses(recipe, batch):
    ...
    """
    torchtune.utils.batch_to_device(batch, recipe._device)
    ...
    decoded_tokens = decode_token_by_token(batch["tokens"],
    ...
    for key, value in additional_logging_losses.items():
        if value.numel() <= 1:
            continue
...
def process_data(
    ...
    try:
        ...
        max_sample_length = recipe._tokenizer.max_seq_len
    ...
    for idx in range(0, len(dataset), batch_size):
        ...
        for key, value in per_token_losses.items():
            if key == "tokens":
                per_token_losses[key] =  value[:, 1:]
            elif key == "decoded_tokens":
                per_token_losses[key] = [v[1:] for v in value]
            else:
                per_token_losses[key] = value[:, :-1]
...
def evaluate_language_generation_length(recipe,
                                        ...
                                        alpha=0.,
                                        equal_entropy=True,
                                        alphabet_size_min=12,
                                        edges_per_state_min=2,
                                        num_states_min=10,
                                        num_states_max=16,
                                        seed=0):
    """
    ...
    The small_models_evaluation measures the length of the longest prefix of the newly
    ...
    Returns:
        Dict[str, Any]: A dictionary containing the small_models_evaluation results, including:
    ...
    kwargs["included_learning_levels"] = [3]
    kwargs["alphabet_size_min"] = alphabet_size_min
    kwargs["edges_per_state_min"] = edges_per_state_min
    kwargs["num_states_min"] = num_states_min
    kwargs["num_states_max"] = num_states_max
    dataset = learning_levels_pfa_dataset(**kwargs)

    char_to_idx = {c: i for i, c in enumerate(dataset.letters)}

    generate_next_token = lambda logits, **kwargs: generate_next_token_mtp_with_blending(logits,
                                                                                         **kwargs,
                                                                           logit_filter=ASCII_LOGIT_MASK,
                                                                                         alpha=alpha,
                                                                                         top_p=top_p,
    ...
    for i in tqdm(range(0, num_samples, batch_size), desc="Evaluating language generation length"):
    # for i in range(0, num_samples, batch_size):
        tokens = []
        transition_symbols = []
        complexities = []
        for j in range(batch_size):
            torch.manual_seed(i + j + seed)
            sample = dataset[i]
            torch.manual_seed(i + j + init_seed)
            tokens.append(sample['tokens'])
            transition_symbols.append(np.array(sample['_transition_symbols'][0]))
            complexity = get_language_complexity(vocab_size=sample['vocab_size'][1],
                                    num_edges=sample['num_edges'][1],
                                    num_states=sample['num_states'][1],
                                    total_tokens=dataset.alphabet_size_max)
            complexities.append(complexity)
            all_vocab_sizes.append(sample['vocab_size'][1])
            all_num_edges.append(sample['num_edges'][1])
            all_num_states.append(sample['num_states'][1])
        generated_sequences = generate_from_recipe(tokens,
                                                   recipe=recipe,
                                                   max_new_tokens=101,
                                                   top_k=top_k,
                                                   stop_tokens=[32],
        ...
        for j, d in enumerate(generated_sequences):
            ...
            for seq_i, seq in enumerate(sequences_np):
                ...
                if seq_i == len(sequences_np) - 1:
                    ...
                    assert accepted
            all_complexities.append(complexities[j])
        current_mean_length = np.array(correct_lenghts).mean()
        # update progress bar
        tqdm.write(f"Current mean correct length: {current_mean_length:.2f}")


    correct_lenghts = correct_lenghts
    all_complexities = all_complexities
    all_vocab_sizes = all_vocab_sizes
    all_num_edges = all_num_edges
    all_num_states = all_num_states
    return {
        "correct_lengths": correct_lenghts,
        "complexities": all_complexities,
        "vocab_sizes": all_vocab_sizes,
        "num_edges": all_num_edges,
        "num_states": all_num_states,
    }
...
@torch.no_grad()
def evaluate_language_generation_creativity(recipe,
                                            num_samples=100,
                                            batch_size=100,
                                            top_k=18,
                                            top_p=1.0,
    ...
    kwargs["num_states_min"] = num_states_min
    kwargs["num_states_max"] = num_states_max
    kwargs["seq_len_min"] = generation_length
    dataset = learning_levels_pfa_dataset(**kwargs)
    ...
    all_num_states = []
    init_seed = torch.initial_seed()
    for i in tqdm(range(0, num_samples), desc="Evaluating language generation length"):
    # for i in range(0, num_samples, batch_size):
        torch.manual_seed(i + seed)
        sample = dataset[i]
        torch.manual_seed(i + init_seed)
        tokens = batch_size * [sample['tokens']]
        transition_symbols = np.array(sample['_transition_symbols'][0])
        complexity = get_language_complexity(vocab_size=sample['vocab_size'][1],
                                num_edges=sample['num_edges'][1],
                                num_states=sample['num_states'][1],
                                total_tokens=dataset.alphabet_size_max)
        ...
        generated_strings = []
        prompt_sequences = generated_sequences[0]['decoded_tokens'].split(' ')[:-1]
        prompt_sequences = [s[:generation_length] for s in prompt_sequences]
        prompt_sequences = set(prompt_sequences)
        for j, d in enumerate(generated_sequences):
            decoded_tokens = d['decoded_tokens']

            sequences = decoded_tokens.split(' ')
            sequences_np = [np.array([char_to_idx[c] if c in char_to_idx else 25 for c in s]) for s in sequences]
            current_symbols = transition_symbols

            for seq_i, seq in enumerate(sequences_np):
                len_correct, accepted = recognized_prefix_length(current_symbols, seq)
                if seq_i == len(sequences_np) - 1:
                    seq_string = sequences[seq_i]
                    if len_correct < generation_length:
                        seq_string = seq_string + '_WRONG'
                    generated_strings.append(seq_string)
                else:
                    assert accepted
        all_complexities.append(complexity)
        unique_strings = set(generated_strings)
        uniqueness_score = len(unique_strings) / len(generated_strings)
        novel_strings = unique_strings - prompt_sequences
        novelty_score = len(novel_strings) / len(unique_strings)
        valid_strings = [s for s in novel_strings if '_WRONG' not in s]
        validity_score = len(valid_strings) / len(novel_strings)
        creativity_score = len(valid_strings) / batch_size

        all_creativity_scores.append(creativity_score)
        all_novelty_scores.append(novelty_score)
        all_uniqueness_scores.append(uniqueness_score)
        all_validity_scores.append(validity_score)
        # update progress bar
        tqdm.write(f"Current mean creativity score: {np.mean(all_creativity_scores):.2f}")
    ...
    all_num_states = all_num_states
    return {
        "creativity_scores": all_creativity_scores,
        "novelty_scores": all_novelty_scores,
        "uniqueness_scores": all_uniqueness_scores,
        "validity_scores": all_validity_scores,
        "complexities": all_complexities,
        "vocab_sizes": all_vocab_sizes,
        "num_edges": all_num_edges,
        "num_states": all_num_states,
...
def pfa_training_evaluation(recipe,
    ...
    Args:
        ...
        ic_generalization_evaluation (bool, optional): If True, runs an additional
            small_models_evaluation to measure the length of correctly generated PFA sequences.
            Defaults to True.
    ...
    Returns:
        ...
        - A dictionary of scalar small_models_evaluation metrics, including
    ...
    elif 'tokenwise_mtp_kl_loss' in datapoints[0]:
        losses.append("tokenwise_mtp_kl_loss")
        losses.append("tokenwise_mtp_ce_loss")
        losses.append("tokenwise_agreement_accuracy")
        interestingness_criterion = "tokenwise_mtp_kl_loss"
    elif "tokenwise_phi_loss" in datapoints[0]:
        losses.append("tokenwise_phi_loss")
        losses.append("tokenwise_mean_mse")
        losses.append("tokenwise_logvar_mse")
        losses.append("tokenwise_entropy_posterior")
        losses.append("tokenwise_entropy_prior")
        interestingness_criterion = "tokenwise_phi_loss"

    if "tokenwise_static_kl_loss" in datapoints[0]:
        losses.append("tokenwise_static_kl_loss")
    ...
    for i, loss in enumerate(losses):
        means = {level: losses_vs_learning_levels_statistics[loss][level]["mean"]
                 for level in levels}
        ...
        fig = go.Figure(data=[go.Bar(x=levels, y=[means[l] for l in levels])])
        # add level names
        fig.update_layout(
