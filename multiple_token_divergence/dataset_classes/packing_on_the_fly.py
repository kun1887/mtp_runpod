...
class PackedOnTheFlyDataset(torch.utils.data.IterableDataset):
    ...
    def __init__(
        ...
        *,
        ...
        self._len = self.estimate_length()

    def estimate_length(self, num_examples_to_load=100):
        """
        Estimate the number of packs that can be created from the dataset. Do that by loading a number of examples and
        counting the number of packs that can be created.
        """
        sampled_lengths = []
        for i in range(num_examples_to_load):
            idx = torch.randint(0, len(self.ds), (1,)).item()
            sample = self.ds[idx]
            seq_len = len(sample['tokens'])
            sampled_lengths.append(seq_len)

        # how many packs can be created from the sampled lengths
        num_packs = 0
        num_tokens = 0
        if not self.split_across_pack:
            for seq_len in sampled_lengths:
            ...
            for seq_len in sampled_lengths:
                if num_tokens + seq_len <= self.max_seq_len:
                    ...
                    num_packs += 1
                    num_tokens = seq_len - (self.max_seq_len - num_tokens)

        examples_per_pack = num_examples_to_load / num_packs if num_packs > 0 else 1
        return int(len(self.ds) / examples_per_pack)
