...
from torchtune.training.checkpointing._utils import ModelType as ModelTypeTorchtune
...
class ModelType(Enum):
    LLAMA3_2 = "llama3_2"
    LLAMA3_VISION = "llama3_2_vision"
    PHI3_MINI = "phi3_mini"
    REWARD = "reward"
    QWEN2 = "qwen2"

    LLAMA3_SELF_PREDICTION = "llama3_self_prediction"
    LLAMA3_MTP = "llama3_mtp"
    LLAMA3_PHI = "llama3_phi"
...
class FullModelTorchTuneCheckpointer(_CheckpointerInterface):
    ...
    def __init__(
        ...
        if checkpoint_dir is not None:
            ...
            self._checkpoint_path = Path(get_path(str(self._checkpoint_dir), checkpoint_files[0]))
...
class FullModelHFCheckpointer(_CheckpointerInterface):
    ...
    def __init__(
        ...
        if checkpoint_dir is not None and checkpoint_files is not None:
            self._checkpoint_dir = Path(checkpoint_dir)
            ...
            self._checkpoint_paths = checkpoint_files = None
        ...
        if self_prediction_checkpoint_dir is not None:
            self._self_prediction_checkpoint_dir = Path(self_prediction_checkpoint_dir)
            self._self_prediction_checkpoint_path = get_path(
                str(self._self_prediction_checkpoint_dir), self_prediction_checkpoint
            )
        else:
            self._self_prediction_checkpoint_path = None
    ...
    def _validate_hf_checkpoint_files(self, checkpoint_files: List[str]) -> List[Path]:
        ...
        for f in checkpoint_files:
            checkpoint_path = get_path(str(self._checkpoint_dir), f)
    ...
    def load_checkpoint(self) -> Dict[str, Any]:
        ...
        if self._model_type == ModelType.LLAMA3_MTP and self._self_prediction_checkpoint_path:
            mtp_state_dict = safe_torch_load(
                self._self_prediction_checkpoint_path
            )
            for key, value in mtp_state_dict.items():
                if not "mtp_layer" in key:
                    key = "mtp_layer." + key
                converted_state_dict[training.MODEL_KEY][key] = value
    ...
    def save_checkpoint(
        ...
        if not adapter_only:
            if self._model_type == ModelType.PHI3_MINI:
                ...
                raise NotImplementedError(
                    "Safe tensors support for LLaMA3 Vision will be added soon. Please use the FullModelMetaCheckpointer for now."
                )
            elif self._model_type == ModelType.LLAMA3_SELF_PREDICTION:
                state_dict[training.MODEL_KEY] = state_dict[training.MODEL_KEY]
            elif self._model_type == ModelType.LLAMA3_MTP:
                state_dict[training.MODEL_KEY] = state_dict[training.MODEL_KEY]
            elif self._model_type == ModelType.LLAMA3_PHI:
                state_dict[training.MODEL_KEY] = state_dict[training.MODEL_KEY]
            else:
