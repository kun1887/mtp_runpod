...
class CheckpointClient:
    ...
    def _save_checkpoint_sync(
        ...
        *,
        dir_prefix: str,
        train_whole_model: bool,
        untrained_parameter_names: List[str],
    ) -> None:
        """
        ...
        if not train_whole_model:
            # This filtering should happen on rank 0 for non-distributed checkpointers,
            # or on all ranks for distributed ones. The logic handles this naturally.
            if self._is_rank_zero or is_distributed_checkpointer:
                log.info(
                    f"train_whole_model is False. Filtering state dict to save only "
                    f"{len(untrained_parameter_names)} trained parameter tensors."
                )
                untrained_set = set(untrained_parameter_names)
                filtered_state_dict = {
                    name: tensor
                    for name, tensor in model_state_dict.items()
                    if name in untrained_set
                }
                model_state_dict = filtered_state_dict  # Overwrite with the filtered dict
        ...
        *,
        ...
        if not full_tensors and self._enable_async_checkpointing:
            ...
            self._save_checkpoint_sync(
                ...
                untrained_parameter_names=untrained_parameter_names,
            )
